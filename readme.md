# Read Me Before Using the Dot Product Algorithm
---

### The Mathematics Behind it: 

$$A.B=|A||B|\cos\theta$$ [^1]
[^1]: The Dot Product Equation

# Links AND Images
[![Google](data:image/png;base64,)](https://www.google.com)
CLICK ME!!

# Headings
## Smaller Heading
### Increase Hashes to Decrease the sizes

*Italic Text*
_Another way_

**Bold Text**
__Another way__

***Italic & Bold Text***
___Another way___
__*Yet Another*__
**_One More_**

Emphasis***In***between
Emphasis___In___between wrong way

##Lists
1. Item 1
2. Nested Unordered list: 
    - 100 Use Escape seqq to begin list heading with a number
    - >Blockquotes in list
        1. List in blockquotes
1. Even if Numbering is wrong in code it implicitly fixes it
2. 

# Code of Dot product
> Traverse through both vectors simultaneimport numpy as np
v1=np.array([1,2,3,4])
v2=np.array([5,6,7,8])
#resolved conflict
ans=v1.dot(v2)
print(ans)


ously and multiply:
```
import numpy as np
v1=np.array([1,2,3,4])
v2=np.array([5,6,7,8])
#resolved conflict
ans=v1.dot(v2)
print(ans)
```

# Footnotes
Lets just refer to this line [^2]
[^2]: Explains how a foot note is created
# Table:
| | A | B | C | 
| ---- | -------- | --------|---- |
| x | 1 | 2 |
| y | -1 | 10 |
| z | | |
|k| 

# Alerts
>[!NOTE]
>This is a Note

>[!TIP]
>This is a Tip
